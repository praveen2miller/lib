...
var posttranscation= async(req,resp)=>{
 ...
 var deleteBook = async (req, resp)=> {
   // const userid = req.params.UserId;
    ...
    try{
        const vBook = await transcationdetails.findOne({where: {BookId: Bookid}});
        console.log(vBook);
        await vBook.destroy();
...
var upadteduedate=async(req,resp)=>{
   ...
   try{
       const bookupdate = await transcationdetails.findOne({where: {UserId:Userid}});
      let date =((new Date(bookupdate.duedate).toISOString()));

     // console.log(date);
      const duedate = await db.sequelize.query("update transcationdetails set `date`=  DATE_ADD(`date`, INTERVAL 16 DAY)", { type: QueryTypes.SELECT
    });
